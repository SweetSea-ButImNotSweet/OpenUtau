            ...
            if (note.lyric.StartsWith("?")) {
                ...
                if (note.lyric.StartsWith("y") && koVVCchia) {
                    ...
                    if (!fry && dem == 2 && kocoC) {
                        ...
                        string dummy = V2;
                        ApDungNguyenTacRieng(loi, V1 + V2, ref V2, ref dummy, ref N, strictToneShift: true);
                        if (tontaiCcuoi) {
                    ...
                    if (!fry && dem == 3 && koVVCchia && kocoC) {
                        ...
                        string N = V3;
                        ApDungNguyenTacRieng(loi, V2 + V3, ref V2, ref V2_2, ref N);
                ...
                if (!fry && dem == 5 && tontaiVVC && tontaiC) {
                    ...
                    N = DecodeVinaConsonants(N);
                    string dummy = VVC;
                    ApDungNguyenTacRieng(loi, V1 + V2, ref VVC, ref dummy, ref N);

                    // BUGFIX: Không gọi lại TinhToanAmChuyenTiep với tham số sai.
                    // Các biến vow / NoVCP / prevtontaiCcuoi đã được tính ở đầu Process().
                    bool hasVCP = !NoVCP;
                    bool hasPrefixVCP = prevtontaiCcuoi;

                    string prefixVCP = hasPrefixVCP ? "- " : vow;
                ...
                if ((dem == 3) && tontaiVVC && kocoC) {
                    ...
                    C = DecodeVinaConsonants(C);
                    string dummy = VVC;
                    ApDungNguyenTacRieng(loi, VVC, ref VVC, ref dummy, ref C);
                    if (prevtontaiCcuoi) vow = "."; else vow += " ";
                    if (NoNext && tontaiCcuoi) {
                        AddPhoneme(phonemes, $"{vow}{V1}");
                        AddPhoneme(phonemes, $"{VVC}", ViTri);
